/**
 * KEV Scorecard Parser
 *
 * Parses KEV Score Cards (Mini and Hybrid formats) used for SNF clinical audits.
 * These have 4 categories: Abuse & Grievances, Accidents & Incidents,
 * Infection Prevention & Control, Skin Integrity & Wounds.
 */
...
// Category sheet mappings for both formats
...
/**
 * Extract facility name, review period, and category scores from cover sheet
 */
function extractCoverInfo(workbook, format) {
  ...
  // Parse month from review period (e.g., "January 2025", "April", etc.)
  let month = null;
  let year = null;
...
/**
 * Parse score value - handles numeric, N/A, and empty values
 */
...
/**
 * Parse a single category sheet
 */
function parseCategorySheet(sheet, categoryName) {
  const data = XLSX.utils.sheet_to_json(sheet, { header: 1 });
  const items = [];

  // Find the header row with "Max Score"
  let maxScoreCol = -1;
  let scoreStartCol = -1;
  ...
  // Parse items - look for rows starting with "1)", "2)", etc.
  for (let row = 0; row < data.length; row++) {
    const rowData = data[row];
    if (!rowData || rowData.length < maxScoreCol + 1) continue;

    // Look for item number pattern in first column
    ...
    if (itemMatch) {
      ...
      // Collect scores from columns after maxScore
      const scores = [];
      ...
      // Calculate points earned (average of valid scores, scaled to max points)
      let pointsEarned = 0;
      let chartsMet = 0;
      let sampleSize = 0;

      if (scores.length > 0) {
        // KEV uses 4-point scale where 4 = full credit, 3 = 75%, etc.
        const avgScore = scores.reduce((a, b) => a + b, 0) / scores.length;
        pointsEarned = (avgScore / 4) * maxPoints;
        chartsMet = Math.round(avgScore * scores.length);
        sampleSize = scores.length * 4; // max possible
      }

      items.push({
        itemNumber,
        criteriaText: criteriaText.substring(0, 200), // Truncate long text
        maxPoints,
        chartsMet,
        sampleSize,
 ...
 */
function parseKevFile(buffer, options = {}) {
  const workbook = XLSX.read(buffer, { type: 'buffer' });

  // Detect format
  const format = detectKevFormat(workbook.SheetNames);
  if (!format) {
    throw new Error('Not a recognized KEV scorecard format');
  }
  ...
  // Get category mappings for this format
  const categoryMappings = CATEGORY_SHEETS[format];

  // Parse each category sheet and use cover sheet scores for totals
  const categories = [];
...
/**
 * Validate parsed KEV data
 */
function validateKevData(parsed) {
  const errors = [];
  const warnings = [];
  ...
  // Check for categories with no items
...
/**
 * Check if a workbook is a KEV format
 */
function isKevFormat(sheetNames) {
